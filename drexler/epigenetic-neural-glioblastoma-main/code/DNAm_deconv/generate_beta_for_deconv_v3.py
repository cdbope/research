...
Version 3: Merges 5mC (m) and 5hmC (h) modifications before extracting beta values.
...
Usage:
    # Single sample
    python generate_beta_for_deconv_v3.py --bedmethyl /path/to/sample.wf_mods.bedmethyl.gz --output_dir results/

    # Batch processing
    python generate_beta_for_deconv_v3.py --input_dir /path/to/bedmethyl_files --sample_ids samples.txt --output_dir results/
...
def process_single_sample(bedmethyl_path, sample_name, probe_coords, atlas_probes, min_coverage, output_dir):
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate beta values CSV from Nanopore bedMethyl for deconvolution (v3 - merges 5mC+5hmC)'
    )
    ...
    print("=" * 60)
    print("Generate Beta Values for Deconvolution (v3 - 5mC+5hmC merged)")
    print("=" * 60)
    ...
    for i, (sample_name, bedmethyl_path) in enumerate(samples, 1):
        ...
        mapped_values = process_single_sample(
            bedmethyl_path, sample_name, epic_coords, atlas_probes, args.min_coverage, args.output_dir
        )
        all_results[sample_name] = mapped_values

        # Track merged file for cleanup
        merged_file = os.path.join(args.output_dir, f"{sample_name}_merged.bedmethyl.gz")
        ...
        # Save full file
        output_file = os.path.join(args.output_dir, f"{sample_name}_deconv_betas_merged.csv")
        df.to_csv(output_file)

        # Save non-zero file
        df_nonzero = df[df[sample_name] > 0]
        output_file_nonzero = os.path.join(args.output_dir, f"{sample_name}_deconv_betas_merged_nonzero.csv")
        df_nonzero.to_csv(output_file_nonzero)

        print(f"    Saved: {sample_name}_deconv_betas_merged.csv ({len(mapped_values)} probes, {len(df_nonzero)} non-zero)")
    ...
    if args.combine and len(samples) > 1:
        ...
        combined_nonzero_file = os.path.join(args.output_dir, "combined_deconv_betas_merged_nonzero.csv")
    ...
    # Cleanup merged bedMethyl files unless --keep_merged is specified
    if not args.keep_merged:
        print("\nCleaning up merged bedMethyl files...")
        for merged_file in merged_files:
            if os.path.exists(merged_file):
                os.remove(merged_file)
        print(f"  Removed {len(merged_files)} merged bedMethyl files")
