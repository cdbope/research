#!/usr/bin/env python3
"""
Version 3: Nanopore Neural Glioblastoma Classification Pipeline

Key changes from v2:
- Uses EPIC_sites_NEW.bed reference file (hg38) instead of liftover coordinates
- Direct probe mapping without R/Bioconductor dependency
- Improved coordinate matching using validated EPIC reference
...
Usage:
    python run_nanopore_classification_v3.py <bedmethyl_file> [--output_dir <dir>]

Example:
    python run_nanopore_classification_v3.py /path/to/T001.wf_mods.bedmethyl.gz --output_dir results/
...
def main():
    parser = argparse.ArgumentParser(
        description='Classify Nanopore methylation data using the Drexler neural glioblastoma classifier (v3 - EPIC reference)'
    )
    ...
    print("=" * 70)
    print("Nanopore Neural Glioblastoma Classification (v3)")
    print("Using EPIC Reference Coordinates + Logistic Regression")
    print("=" * 70)
    ...
    # Report beta value statistics before imputation
    valid_betas = df.loc[sample_name].dropna()
    print(f"\nBeta value statistics (before imputation):")
    print(f"  Mean: {valid_betas.mean():.4f}")
    print(f"  Median: {valid_betas.median():.4f}")
    print(f"  Std: {valid_betas.std():.4f}")
    print(f"  Min: {valid_betas.min():.4f}")
    print(f"  Max: {valid_betas.max():.4f}")

    # Report high vs low probe statistics
    high_probe_betas = df.loc[sample_name, [c for c in high_cgs if c in df.columns]].dropna()
    low_probe_betas = df.loc[sample_name, [c for c in low_cgs if c in df.columns]].dropna()
    print(f"\n  High-neural probes ({len(high_probe_betas)}): mean={high_probe_betas.mean():.4f}")
    print(f"  Low-neural probes ({len(low_probe_betas)}): mean={low_probe_betas.mean():.4f}")

    # Save beta values
    betas_file = os.path.join(args.output_dir, f"{sample_name}_betas.csv")
    df.T.to_csv(betas_file)
    print(f"\nSaved beta values to: {betas_file}")
    ...
    print("\n" + "=" * 70)
    print("Step 5: Running Logistic Regression classification")
    print("=" * 70)
