...
if (nrow(samples_df) == 0) {
  ...
  if (ncol(samples_df) >= 2) {
    ...
    cat("The estimated tumor content file has insufficient columns. Expected at least 2 columns.\n")
  }
}
...
tryCatch({
    ...
    if (nrow(df_mgmt) == 0) {
        ...
        if ("mean_methylation_full" %in% colnames(df_mgmt)) {
            df_mgmt$mean_methylation_full <- round(as.numeric(df_mgmt$mean_methylation_full) * 100, 1)
        }
        if ("mean_methylation_pyro" %in% colnames(df_mgmt)) {
            df_mgmt$mean_methylation_pyro <- round(as.numeric(df_mgmt$mean_methylation_pyro) * 100, 1)
        }
